...
var CollectionFacade = function(db, collection) {
	fargs(arguments)
...
var collectionFacadeChain = chainbuilder.chain(CollectionFacade.prototype);

// create possible commands
['insert', 'remove', 'update', 'count', 'find', 'findOne', 'drop'].forEach(function(command){
	
	collectionFacadeChain.operation(command, function() {
...
['limit','skip','sort','fields','timeout'].forEach(function(option) {
	...
	collectionFacadeChain.option(option, function(operation, value) {
...
collectionFacadeChain.finalizeWith("end");
