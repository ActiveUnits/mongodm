...
var CollectionFacade = function(db) {
	this.db = db;
...
chain.defineMethod("allocateWithPool", false, function(collections, name, callback){
	fargs(arguments)
		.required(new Error("collections pool required"))
	...
	if(collections.get(name) == null) {
		...
		this.collection = collections.get(name);
		if(callback)
...
['insert', 'remove', 'update', 'count', 'find', 'findOne', 'drop'].forEach(function(command){
	...
	chainMethod = chain.defineMethod(command, function() {
					...
					return function(err, cursor){
						if(err == null)
							cursor.toArray(function(err, results){
								augmentedResultHandler(f)(err, results);
							});					
						else
							if(f)
								f(err,null);
		...
		if(command == "drop") {
			...
			this.db.dropCollection(this.collection.collectionName, arguments[arguments.length-1]);
			this.collection = null;
