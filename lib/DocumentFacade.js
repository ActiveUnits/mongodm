...
module.exports = function(db, documentDefinition){
	...
	DocumentFacade.fromJSON = function(json) {
		...
		sys.log("instance from JSON: "+sys.inspect(instance));
		return instance;
	};
	...
	DocumentFacade.prototype.save = function(callback) {
		...
		if(this._id == null) {
			...
			//sys.log("UPDATING "+sys.inspect(this.toJSON(true)));
			this.withCollection()
	...
	DocumentFacade.prototype.toJSON = function(onlyDirtyFields) {
		...
		var result = null;
		if(onlyDirtyFields) {
			result = jsonTools.diff(this.fields, this.stagedFields);
			if(result.diffs)
			...
			result = jsonTools.clone(this.fields);
