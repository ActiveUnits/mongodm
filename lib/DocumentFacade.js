...
module.exports = function(dbfacade, documentDefinition){
	...
	var updateFieldsFromFunction = function(obj, keepFields) {
		if(typeof keepFields == "undefined")
			keepFields = [];
		
		for(var i in obj) {
			if(keepFields.indexOf(i) != -1)
				continue;
			
			if(i == "_id" && typeof obj[i] !== "object") {
				this[i] = dbfacade.toMongoID(obj[i]);
			} else {
				...
				if(typeof obj[i] == "object")
					updateFieldsFromFunction.apply(this[i], [obj[i]]);
				else
					this[i] = obj[i];
			}
		}
		...
		if(this._id == null) {
			...
			_self.withCollection()
				 .insert(jsonTools.clone(this, ["__stagedFields__", "_id"]), function(err, docs){
					 ...
					 if(callback)
						 callback(err, _self); // return always self instance assuming it is in sych with the DB stored.
				 });
		} else {
	...
	var facadeDocumentObject = function(obj) {
		...
		_self.updateFieldsFrom = updateFieldsFromFunction;
	...
	DocumentFacade.find = function(pattern,options,callback) {
		...
		if(pattern["_id"] && typeof pattern["_id"] != "object")
    		if(/^[0-9a-fA-F]{24}$/.test(pattern["_id"]))
	    	    pattern["_id"] = dbfacade.toMongoID(pattern["_id"]);
