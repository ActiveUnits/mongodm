...
chain.defineMethod("defineDocument", function(documentDefinition, name, callback){
	fargs(arguments)
		.required("at least one argument (doc definition) is required")
		.skipAsValue(null)
		.skipAsFunction(null)
		.end();
	if(!name)
	...
	if(!name) {
		callback(new Error("can not define document without name. use defineDocument(defition, name), or set name field within defition"));
		return;
	}
	
	this.withDocument(name, documentDefinition, callback);
});
...
chain.defineFork("toMongoID", function(id) {
