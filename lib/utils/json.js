...
var jsonPrimitives = {
	diff : function(current, history, ignoreFields) {
		...
		if(typeof current == "undefined")
			throw new Error("can not diff current as it is undefined");
		...
		if(typeof history == "undefined") {
			...
			if(history) {
				...
				for(var key in current) {
					if(ignoreFields.indexOf(key) == -1 && typeof current[key] != "function") {
	...
	clone : function(input,ignoreFields) {
		...
		if(typeof input == "object" && input instanceof Date) {
			...
			var result = {};
			for(var key in input)
				if(ignoreFields.indexOf(key) == -1 && typeof input[key] != "function")
